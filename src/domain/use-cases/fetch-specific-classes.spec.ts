...
import { FetchSpecificClassesUseCase } from './fetch-specific-classes'

let inMemoryInstitutionRepository: InMemoryInstitutionRepository
let inMemoryClassRepository: InMemoryClassRepository
let sut: FetchSpecificClassesUseCase

describe('Fetch Specific Classes', () => {
  beforeEach(() => {
    inMemoryInstitutionRepository = new InMemoryInstitutionRepository()
    inMemoryClassRepository = new InMemoryClassRepository()
    sut = new FetchSpecificClassesUseCase(inMemoryClassRepository)
  })

  it('should to be able fetch specific classes', async () => {
